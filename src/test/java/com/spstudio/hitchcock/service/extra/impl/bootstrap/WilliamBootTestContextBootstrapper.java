package com.spstudio.hitchcock.service.extra.impl.bootstrap;
...
public class WilliamBootTestContextBootstrapper extends DefaultTestContextBootstrapper {
	...
	private static final Log logger = LogFactory.getLog(WilliamBootTestContextBootstrapper.class);
	...
	public TestContext buildTestContext() {
		...
		AutowireCapableBeanFactory beanFactory = context.getApplicationContext().getAutowireCapableBeanFactory();
		NoPurposeBean bean = beanFactory.createBean(NoPurposeBean.class);
		
		if (beanFactory instanceof ConfigurableListableBeanFactory) {
			ConfigurableListableBeanFactory listableBeanFactory = (ConfigurableListableBeanFactory)beanFactory;
			listableBeanFactory.registerSingleton(NoPurposeBean.class.getCanonicalName(), bean);
		}
	...
	protected Class<?>[] getOrFindConfigurationClasses(MergedContextConfiguration mergedConfig) {
		...
		Assert.state(found != null, "Unable to find a @SpringBootConfiguration, you need to use "
				+ "@ContextConfiguration or @WilliamBootTest(classes=...) " + "with your test");
	...
	protected WebEnvironment getWebEnvironment(Class<?> testClass) {
		WilliamBootTest annotation = getAnnotation(testClass);
	...
	protected WilliamBootTest getAnnotation(Class<?> testClass) {
		return AnnotatedElementUtils.getMergedAnnotation(testClass, WilliamBootTest.class);
	}

	protected void verifyConfiguration(Class<?> testClass) {
		WilliamBootTest WilliamBootTest = getAnnotation(testClass);
		if (WilliamBootTest != null
				&& (WilliamBootTest.webEnvironment() == WebEnvironment.DEFINED_PORT
						|| WilliamBootTest.webEnvironment() == WebEnvironment.RANDOM_PORT)
			...
			throw new IllegalStateException("@WebAppConfiguration should only be used "
					+ "with @WilliamBootTest when @WilliamBootTest is configured with a "
					+ "mock web environment. Please remove @WebAppConfiguration or " + "reconfigure @WilliamBootTest.");
		}
	}
